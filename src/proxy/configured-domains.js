...
const proxies = new Map(
  Array.from(domains).map(domain => {
    return [
      domain,
      createDefaultProxy(`https://${domain}`, {
...
export function enableDefaultProxy(expressApp) {
  expressApp.use("/", (req, res, next) => {
    if (
      !req.headers ||
      !req.headers.host ||
      !config.hostsWhitelistRegex.test(req.headers.host)
    ) {
      info(
        `FORBIDDEN: proxy request from host "${req.headers.host}" was canceled as it doesn't match with ${config.hostsWhitelistRegex}.`
      );
